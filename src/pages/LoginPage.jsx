...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await login(email, password); // Simula login
      toast({
  ...
  const handleGoogleLogin = () => {
    ...
    toast({
      ...
      description: 'Funcionalidade de login com Google será implementada com Supabase.',
    });
    // Simulação de login com Google
    setTimeout(() => {
      // login(email, password) // Aqui chamaria o login do Google via Supabase
      // navigate('/dashboard');
      setIsLoading(false);
    }, 1500);
  };
      ...
      >
        <div className="text-center">
          <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary via-purple-500 to-pink-500">
            ChatPro Interplase AI ✨
          </h1>
          <p className="mt-2 text-muted-foreground">Bem-vindo de volta! Faça login para continuar.</p>
        </div>
        ...
        <p className="text-center text-sm text-muted-foreground">
          ...
          <Link to="/signup" className="font-medium text-primary hover:underline">
            Cadastre-se <ExternalLink className="inline h-4 w-4" />
          </Link>
        </p>
      </motion.div>
    </div>
